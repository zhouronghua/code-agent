...
## 0. 最新变更：保底模型可见化 + 恢复探活 + 切换上下文一致性 (v0.3.36)

> **时间**: 2026-09-15 | **变更**: 模型切换可观测、后台探活自动切回、跨模型上下文修复

### 问题定位（基于真实 session / task log 证据）

最近 session 里出现大量 `Let me do it` 之类的「废话」，定位结论：**不是 prompt 变更，而是保底模型
在自我重复**。证据来自 `~/.codeagent/tasks/task_1789444306501_2dspe8.json`：

- task log 末尾 `config.model = hy3`，即该任务在中途因超时切到了**保底模型 hy3**；
- step 34 单步 `reasoningContent` 达 **98,320 字符**，其中 `Let me do it.` 重复 **1617 次**，
  `completionTokens = 32768` 恰好等于 hy3 的 `maxOutputTokens`，即**思考把输出预算烧光**；
- 该步 `content` 为空、`toolCalls` 为空，而 agent loop 把它当成「最终回答」直接结束任务，
  用户只看到一屏 `THINKING` 废话 + 一句空回答，只能手敲「继续」。

同时发现「模型一切换，上下文就不对了」的根因：切换只换了 provider，**没有换窗口预算**。
实测同任务 step 19 的 `promptTokens` 从 497k 掉到 113k、`cachedTokens` 归零——正是小窗口模型
承接大窗口历史后触发强制压缩的表现。

### 变更内容

1. **模型切换可见**：每次切换打印 `[MODEL] ...`（`fallback-timeout` / `primary-recovered` /
   `routing` / `profile`），并写入 task log 的 `modelSwitches` 字段。
2. **后台恢复探活**：切到保底模型后按 30s→120s 退避探测默认模型（`healthCheck`，单个极小
   completion），**一恢复立即切回**；定时器 `unref()` 不阻塞进程；进行中的请求不被中途打断。
   实测网关行为：`max_tokens: 1` 会被 GPT-5.x 拒为 400（会误判不可达），故探针用 8；
   未知模型返回 503，据此确定「2xx/400/429 可达、5xx/401/403/404/超时 不可达」的判定策略。
3. **切换时上下文一致**：`AgentContext` 的窗口预算跟随新模型（防止大窗口历史压垮小窗口模型而
   被强制压缩）；切到非 thinking 模型时丢弃上一个模型的 `reasoning_content`（保留
   `content`/`tool_calls`），即修复「模型一换上下文就不对」。
4. **思考死循环兜底**：识别「同一行高比例重复」的退化推理，丢弃该段思考并注入纠正指令重试
   （最多 2 轮）；空答案一律不再被当作最终回答。
5. **控制台思考截断**：`THINKING` 超过 4000 字符按首尾截断并标注省略量。
6. **保底模型改用 `gpt-5.6-luna`**（非 thinking，200k/64k），全局配置与模板同步更新。

### 验证

`tests/model-switch.test.ts`：63 项断言全通过（退化推理识别、切换打印、跨模型上下文、
保底回退 + 探活切回、路由解析、真实网关健康判定策略、死循环恢复）。
另用本地 mock 网关对**已安装的 v0.3.36** 做了端到端验证：主模型 504 → 打印
`[MODEL] ⚠ ... switched to 保底模型 ...` → 保底模型接管 → 探针发现主模型恢复 →
打印 `[MODEL] ✓ ... switched back ...` → 回复来自主模型（`ANSWER-FROM-PRIMARY-MODEL`）。
真实网关实测 `gpt-5.6-luna healthCheck=true`。

...
- `/btw <hint>` -- agent 运行中插入提示；空闲时作为直接任务执行
- `/btw cancel` -- 取消当前正在执行的工具调用（长构建 / poll 等）
- `exit` / `quit` -- 退出

#### 中途干预 `/btw`：提示 vs. 取代任务

agent 运行中发送 `/btw <指令>` 时会先**判断这条指令与正在执行的任务是否冲突**：

- **补充提示**（加约束、提醒、补充说明）：保留当前计划，提示注入到后续步骤。
- **取代任务**（改了目标 / 要求基于最新版本重跑 / 让当前工作作废）：立即
  `abort` 正在执行的工具调用（长构建、poll、CI 等待），丢弃当前计划，把新指令
  作为新任务重新规划——不会先把旧的长时间任务跑完再读这条指令。

判定顺序：先看确定性关键词（如「改为 / 改成 / 重来 / 重新执行 / 基于最新版本 /
算了 / 放弃 / instead / start over」→ 取代；「注意 / 记得 / 补充 / 另外 / note」
→ 提示），无法判定时再用一次轻量 LLM 调用评估。评估超时或失败一律**降级为提示**，
绝不误杀正在跑的工作。

系统会记录 `task-superseded` 信号到任务日志（`~/.agent/tasks/`），便于事后审计。
